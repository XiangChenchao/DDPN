...
#include "caffe/layers/my_tile_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void MyTileLayer<Dtype>::Reshape(
  ...
  tiles_ = bottom[1]->shape(bottom[1]->CanonicalAxisIndex(tile_param.axis()));
...
template <typename Dtype>
void MyTileLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void MyTileLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(MyTileLayer);
#endif

INSTANTIATE_CLASS(MyTileLayer);
REGISTER_LAYER_CLASS(MyTile);
